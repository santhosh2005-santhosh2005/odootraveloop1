...
function runMigrations(db: Database.Database): void {
  ...
  const migrations: Migration[] = [
    ...
    () => {
      try { db.exec('ALTER TABLE places ADD COLUMN cost_index INTEGER'); } catch (err: any) { if (!err.message?.includes('duplicate column name')) throw err; }
      try { db.exec('ALTER TABLE places ADD COLUMN popularity_score REAL'); } catch (err: any) { if (!err.message?.includes('duplicate column name')) throw err; }
    },
    () => {
      try { db.exec('ALTER TABLE trips ADD COLUMN budget_limit REAL'); } catch (err: any) { if (!err.message?.includes('duplicate column name')) throw err; }
    },
    () => {
      try { db.exec('ALTER TABLE places ADD COLUMN activity_type TEXT'); } catch (err: any) { if (!err.message?.includes('duplicate column name')) throw err; }
    },
  ];
