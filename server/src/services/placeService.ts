...
export function createPlace(
  ...
  const {
    ...
    cost_index, popularity_score,
    transport_mode, activity_type, tags = [],
  } = body;
  ...
  const result = db.prepare(`
    INSERT INTO places (trip_id, name, description, lat, lng, address, category_id, price, currency,
      place_time, end_time,
      duration_minutes, notes, image_url, google_place_id, osm_id, website, phone,
      cost_index, popularity_score, transport_mode, activity_type)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    ...
    google_place_id || null, osm_id || null, website || null, phone || null,
    cost_index || null, popularity_score || null, transport_mode || 'walking', activity_type || null,
  );
...
export function updatePlace(
  ...
  const existingPlace = db.prepare('SELECT * FROM places WHERE id = ? AND trip_id = ?').get(placeId, tripId) as any;
  ...
  `).run(
    ...
    cost_index !== undefined ? cost_index : existingPlace.cost_index,
    popularity_score !== undefined ? popularity_score : existingPlace.popularity_score,
    transport_mode || null,
    placeId,
  );
