...
export function getAnalytics() {
  const topCities = db.prepare(`
    SELECT region_name as city, country_code, COUNT(*) as visit_count
    FROM place_regions
    JOIN places ON places.id = place_regions.place_id
    GROUP BY region_name, country_code
    ORDER BY visit_count DESC
    LIMIT 10
  `).all() as { city: string; country_code: string; visit_count: number }[];

  const tripsByMonth = db.prepare(`
...
export async function getGithubReleases(perPage: string = '10', page: string = '1') {
  try {
    const resp = await fetch(
      `https://api.github.com/repos/santhosh/trek/releases?per_page=${perPage}&page=${page}`,
...
export async function checkVersion(): Promise<VersionInfo> {
  ...
  try {
    if (isPrerelease) {
      ...
      const resp = await fetch(
        'https://api.github.com/repos/santhosh/trek/releases?per_page=100',
        { headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'TREK-Server' } }
      );
      if (!resp.ok) {
        return fallback;
      ...
      const resp = await fetch(
        'https://api.github.com/repos/santhosh/trek/releases/latest',
        { headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'TREK-Server' } }
      );
      if (!resp.ok) {
        return fallback;
