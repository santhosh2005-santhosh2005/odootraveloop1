...
export function createTrip(userId: number, data: CreateTripData, maxDays?: number) {
  ...
  const result = db.prepare(`
    INSERT INTO trips (user_id, title, description, start_date, end_date, currency, budget_limit, reminder_days)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(userId, data.title, data.description || null, data.start_date || null, data.end_date || null, data.currency || 'EUR', data.budget_limit || null, rd);
...
export function updateTrip(tripId: string | number, userId: number, data: UpdateTripData, userRole: string): UpdateTripResult {
  ...
  const { title, description, start_date, end_date, currency, budget_limit, is_archived, cover_image, reminder_days } = data;
  ...
  const newBudgetLimit = budget_limit !== undefined ? budget_limit : (trip as any).budget_limit;
  ...
  db.prepare(`
    UPDATE trips SET title=?, description=?, start_date=?, end_date=?,
      currency=?, budget_limit=?, is_archived=?, cover_image=?, reminder_days=?, updated_at=CURRENT_TIMESTAMP
    WHERE id=?
  `).run(newTitle, newDesc, newStart || null, newEnd || null, newCurrency, newBudgetLimit, newArchived, newCover, newReminder, tripId);
