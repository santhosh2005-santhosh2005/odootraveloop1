...
const UA = 'TREK Travel Planner (https://github.com/Santhoshboe/TREK)';
...
export async function searchNominatim(query: string, lang?: string) {
  ...
  return data.map(item => ({
    ...
    website: null,
    phone: null,
    cost_index: (item.display_name?.length || 0) % 4 + 1, // Mock cost index 1-4
    popularity_score: ((item.name?.length || 0) % 50) / 10, // Mock popularity 0-5
    estimated_duration: (item.name?.length || 0) % 3 === 0 ? 'short' : ((item.name?.length || 0) % 3 === 1 ? 'medium' : 'long'), // Mock duration
...
export async function searchPlaces(userId: number, query: string, lang?: string, filters?: { type?: string; duration?: string }): Promise<{ places: Record<string, unknown>[]; source: string }> {
  const apiKey = getMapsKey(userId);

  if (!apiKey) {
    let osmQuery = query;
    if (filters?.type && filters.type !== 'all') {
      osmQuery += ` ${filters.type}`;
    }
    let places = await searchNominatim(osmQuery, lang);
    if (filters?.duration && filters.duration !== 'all') {
      places = places.filter(p => p.estimated_duration === filters.duration);
    }
    return { places, source: 'openstreetmap' };
  }

  const body: any = { textQuery: query, languageCode: lang || 'en' };

  if (filters?.type && filters.type !== 'all') {
    // Map internal types to Google Places types
    const typeMap: Record<string, string[]> = {
      'restaurant': ['restaurant', 'cafe', 'bar'],
      'sightseeing': ['tourist_attraction', 'museum', 'park', 'church', 'hindu_temple', 'mosque'],
      'adventure': ['amusement_park', 'hiking_area', 'national_park', 'zoo'],
      'shopping': ['shopping_mall', 'department_store', 'clothing_store'],
    };
    if (typeMap[filters.type]) {
      body.includedType = typeMap[filters.type][0]; // Text search only supports one includedType in basic mode
    }
  }
  ...
  let places = (data.places || []).map((p: GooglePlaceResult) => {
    const cost_index = (p.displayName?.text?.length || 0) % 4 + 1;
    const popularity_score = p.rating || ((p.displayName?.text?.length || 0) % 50) / 10;
    
    // Estimate duration based on types
    let estimatedDuration: 'short' | 'medium' | 'long' = 'medium';
    if (p.types?.some(t => ['bakery', 'cafe', 'fast_food', 'atm', 'convenience_store'].includes(t))) estimatedDuration = 'short';
    if (p.types?.some(t => ['museum', 'art_gallery', 'zoo', 'amusement_park', 'stadium'].includes(t))) estimatedDuration = 'long';
