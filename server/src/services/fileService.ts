...
export function formatFile(file: TripFile & { trip_id?: number }) {
  ...
  // If the filename starts with 'files/', it's a Supabase path
  const isSupabase = file.filename.startsWith('files/');
  return {
    ...file,
    url: isSupabase ? getPublicUrl(file.filename) : `/api/trips/${tripId}/files/${file.id}/download`,
    uploaded_by_avatar: file.uploaded_by_avatar ? (file.uploaded_by_avatar.startsWith('avatars/') ? getPublicUrl(file.uploaded_by_avatar) : `/uploads/avatars/${file.uploaded_by_avatar}`) : null,
  };
}
...
export async function createFile(
  ...
  // Upload to Supabase
  const { path: storagePath } = await uploadFile(file, 'files');
