...
export async function checkBudgetAlert(tripId: string | number) {
  const trip = db.prepare('SELECT id, title, budget_limit, currency FROM trips WHERE id = ?').get(tripId) as { id: number; title: string; budget_limit: number | null; currency: string } | undefined;
  if (!trip || trip.budget_limit === null) return;

  const totalSpent = db.prepare('SELECT SUM(total_price) as total FROM budget_items WHERE trip_id = ?').get(tripId) as { total: number | null };
  const total = totalSpent.total || 0;

  if (total > trip.budget_limit) {
    await send({
      ...
      params: {
        ...
        limit: String(trip.budget_limit.toFixed(2)),
