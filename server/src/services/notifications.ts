...
const EVENT_TEXTS: Record<string, Record<NotifEventType, EventTextFn>> = {
  en: {
    ...
    budget_limit_exceeded: p => ({ title: `Budget exceeded: ${p.trip}`, body: `Your budget for "${p.trip}" has been exceeded! Current spending: ${p.total} ${p.currency} (Limit: ${p.limit} ${p.currency}).` }),
  ...
  de: {
    ...
    budget_limit_exceeded: p => ({ title: `Budget überschritten: ${p.trip}`, body: `Dein Budget für "${p.trip}" wurde überschritten! Aktuelle Ausgaben: ${p.total} ${p.currency} (Limit: ${p.limit} ${p.currency}).` }),
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 480px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        ...
        <tr><td style="padding: 20px 32px; background: #f9fafb; border-top: 1px solid #f3f4f6; text-align: center;">
          ...
          <p style="margin: 0; font-size: 10px; color: #d1d5db;">${s.madeWith} <span style="color: #ef4444;">&hearts;</span> by Santhosh &middot; <a href="https://github.com/Santhoshboe/Trek" style="color: #9ca3af; text-decoration: none;">GitHub</a></p>
        </td></tr>
      </table>
    </td></tr>
  </table>
