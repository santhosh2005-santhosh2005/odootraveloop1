...
function BagCard({ bag, bagItems, totalWeight, pct, tripId, tripMembers, canEdit, onDelete, onUpdate, onSetMembers, t, compact }: BagCardProps) {
  ...
  const handleResetChecklist = () => {
    ...
    if (!confirm(t('packing.confirm.resetChecklist'))) return
    items.forEach(item => {
      if (item.checked) {
        packingApi.updateItem(tripId, item.id, { checked: false }).catch(() => {})
      }
    })
  }
            ...
            {inlineHeader && canEdit && abgehakt > 0 && (
              <>
                <button onClick={handleResetChecklist} style={{
                  fontSize: 11.5, padding: '5px 10px', borderRadius: 99, border: '1px solid var(--border-primary)',
                  background: 'var(--bg-card)', color: 'var(--text-muted)', cursor: 'pointer', fontFamily: 'inherit',
                  marginRight: 6,
                }}>
                  <span className="hidden sm:inline">{t('packing.resetChecklist')}</span>
                </button>
