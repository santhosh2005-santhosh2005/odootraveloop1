  ...
  useEffect(() => {
    if (trip) {
      ...
      setFormData({
        ...
        budget_limit: trip.budget_limit || null,
      })
      setCustomReminder(![0, 1, 3, 9].includes(rd))
      setCoverPreview(trip.cover_image || null)
    } else {
      setFormData({ title: '', description: '', start_date: '', end_date: '', reminder_days: tripRemindersEnabled ? 3 : 0, day_count: 7, budget_limit: null })
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4" onPaste={handlePaste}>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('dashboard.budgetLimit')}</label>
          <input type="number" step="0.01" value={formData.budget_limit || ''}
            onChange={e => canEditTrip && update('budget_limit', e.target.value ? parseFloat(e.target.value) : null)}
            readOnly={!canEditTrip} placeholder={t('dashboard.budgetLimitPlaceholder')} className={inputCls} />
        </div>
