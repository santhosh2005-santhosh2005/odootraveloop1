  ...
  return (
    ...
    >
      <div style={{
        ...
        {/* Metadata: Cost and Popularity */}
        {(place.cost_index || place.popularity_score !== undefined || googleDetails?.cost_index || googleDetails?.popularity_score !== undefined) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, padding: '0 16px' }}>
            {(place.cost_index || googleDetails?.cost_index) && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <Euro size={14} style={{ color: 'var(--text-faint)' }} />
                <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--brand-primary)' }}>
                  {'$'.repeat(place.cost_index || googleDetails?.cost_index)}
                </span>
              </div>
            )}
            {(place.popularity_score !== undefined && place.popularity_score !== null || googleDetails?.popularity_score !== undefined && googleDetails?.popularity_score !== null) && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <TrendingUp size={14} style={{ color: 'var(--text-faint)' }} />
                <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-primary)' }}>
                  ★ {(place.popularity_score ?? googleDetails?.popularity_score ?? 0).toFixed(1)}
                </span>
              </div>
            )}
          </div>
