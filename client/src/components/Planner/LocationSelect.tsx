...
export default function LocationSelect({ value, onChange, placeholder, style }: Props) {
  ...
  const pick = (r: any) => {
    ...
    const loc: any = {
      name: r.name || r.address || 'Location',
      lat,
      lng,
              ...
              <span style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                    {r.cost_index && (
                      <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--brand-primary)' }}>
                        {'$'.repeat(r.cost_index)}
                      </span>
                    )}
                    {r.popularity_score !== undefined && r.popularity_score !== null && (
                      <span style={{ fontSize: 10, background: 'var(--bg-tertiary)', padding: '1px 4px', borderRadius: 4, color: 'var(--text-faint)' }}>
                        ★ {r.popularity_score.toFixed(1)}
                      </span>
                    )}
                    {r.estimated_duration && (
                      <span style={{ fontSize: 10, background: 'var(--bg-tertiary)', padding: '1px 4px', borderRadius: 4, color: 'var(--text-faint)' }}>
                        {r.estimated_duration === 'short' ? '< 1h' : (r.estimated_duration === 'medium' ? '1-3h' : '3h+')}
                      </span>
                    )}
                  </div>
                </div>
