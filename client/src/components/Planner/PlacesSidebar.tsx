...
const PlacesSidebar = React.memo(function PlacesSidebar({
  ...
  const filtered = useMemo(() => places.filter(p => {
    ...
    if (activityFilter !== 'all' && p.activity_type !== activityFilter) return false
    
    // Duration Filter
    if (durationFilter !== 'all') {
      const mins = p.duration_minutes || 60
      if (durationFilter === 'short' && mins >= 60) return false
      if (durationFilter === 'medium' && (mins < 60 || mins > 180)) return false
      if (durationFilter === 'long' && mins <= 180) return false
    }

    // Cost Index Filter
    if (costFilter !== 'all') {
      const cost = p.cost_index || 0
      if (costFilter === 'low' && cost > 1) return false
      if (costFilter === 'medium' && (cost <= 1 || cost > 3)) return false
      if (costFilter === 'high' && cost <= 3) return false
    }
    ...
    return true
  }), [places, filter, categoryFilters, search, plannedIds, activityFilter, durationFilter, costFilter])
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '14px 16px 10px', borderBottom: '1px solid var(--border-faint)', flexShrink: 0 }}>
        ...
        {/* Advanced Filters */}
        <div style={{ display: 'flex', gap: 4, marginBottom: 8, overflowX: 'auto', paddingBottom: 2, scrollbarWidth: 'none' }}>
          <select
            value={activityFilter}
            onChange={e => setActivityFilter(e.target.value)}
            style={{
              fontSize: 10, padding: '3px 6px', borderRadius: 6, border: '1px solid var(--border-primary)',
              background: 'var(--bg-card)', color: 'var(--text-primary)', outline: 'none', cursor: 'pointer'
            }}
          >
            <option value="all">{t('places.activityType')}</option>
          ...
          >
            <option value="all">{t('places.costIndex')}</option>
